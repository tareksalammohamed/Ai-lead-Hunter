...
export async function getAdminAIProviders(): Promise<AdminAIProvider[]> {
  ...
  const seed: AdminAIProvider[] = [
    ...
    { id: generateId(), provider: 'cerebras', enabled: false, api_key_masked: '', priority: 4, default_model: 'gpt-oss-120b', fallback_enabled: true, max_requests: 1000, timeout_ms: 30000, retry_count: 3, base_url: 'https://api.cerebras.ai/v1', model_fallback_chain: ['gpt-oss-120b'], updated_at: nowISO() },
